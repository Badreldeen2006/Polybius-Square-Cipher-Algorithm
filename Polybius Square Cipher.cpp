...
void do_matrix()
{
    ...
    while (true)
    {
        ...
        bool end_while_2 = true; // matrix must contain all digits 1~5
        for (int i = 0; i < 5; i++)
        {
            for (int j = i + 1; j < 5; j++)
            {
                if (int_key_digt[i] == int_key_digt[j])
                {
                    cout << " (( Enter a valid key *contain all dgts 1 ~ 5*, no duplicates ))" << endl;
                    end_while_2 = false;
                    break;
                }
            }
            if (end_while_2 == false)
                break;
        }
...
void cipher_Polybius()
{
    while (true)
    {
        ...
        for (char check : message)
        {
            check = toupper(check);
            if (check == 'J')
            { //  convert the J letter to I
                check = 'I';
            }
            if (isalpha(check))
            { // fill the fixed message
                fixed_message += check;
            }
        }
        ...
        else
        {
            ...
            for (int i = 0; i < len; i++)
            { // to check all digits in the message
                for (int row = 1; row < 6; row++)
                { // to check current digit in the message if in the current row or not
                    for (int col = 1; col < 6; col++)
                    { // to check current digit in the message if in the current row and col or not
...
void decipher_Polybius()
{
    while (true)
    {
        ...
        for (char check : message)
        { // to fix the message and ignore any input not digit
            if (isdigit(check))
            {
                fixed_message += check;
            }
        ...
        if (fixed_message.size() % 2 != 0)
        { // to make sure the message digits is even number
        ...
        else
        {
            ...
            {
                for (int j = 0; j <= 5; j++)
                { // j is equal to  matrix[i][0] that is the key
            ...
            {
                for (int j = 0; j <= 5; j++)
                { // j is equal to  matrix[0][i] that is the key
